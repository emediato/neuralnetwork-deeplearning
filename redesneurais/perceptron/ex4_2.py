...
plt.plot(x1_reta, x2_reta, c='black')
plt.show()
...
# Creating a mesh grid of X and T
#T1,X1 = np.meshgrid(xc1[:,0], xc1[:,1])
#Z1=X1+T1
#X2, Y2 = np.meshgrid(xc2[:,0], xc2[:,1])
#Z2=X2+Y2

#T, X = np.meshgrid(t, x)
#ax.plot_surface(X1,T1,U)
ax.plot(xc1[:,0], xc1[:,1], zs=0, zdir='z')
#ax.plot_surface(xc1[:,0], xc1[:,1], U, zs=0, zdir='y')
